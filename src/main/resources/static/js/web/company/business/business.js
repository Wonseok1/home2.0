...
var tspan_delta = 16; //name is used as the title for the bubble
...
var use_copy = document.querySelector("use.nav-copy"); //Keeps code DRY avoiding namespace for element creation
...
function addService(serv, index) {
    ...
    var service_bubble = jQuery(".serv-" + index); //Uses tween to look for right position
...
function createPointer() {
    ...
    twn_pointer_path.to(service_pointer, 1.7,{
        bezier: {
...
function serviceClick(ev) {
    ...
    current.addClass("active"); //There's a "serv-*" class for each bubble

    var current_class = current.attr("class").split(" ")[1];
    var class_index = current_class.split('-')[1]; //Hides current text of the main bubble

    jQuery(use_copy).addClass("changing"); //Sets pointer to the right position

    twn_pointer_path.tweenTo(class_index * (pointer_time / (2 * 6))); //After it's completely hidden, the text changes and becomes visible
...
var twn_pivot_path = TweenMax.to(pivot_path, 10, {
    bezier: {
        values: half_circle,
        curviness: 1.5
    },
...
services_data.reduce(function (count, serv) {
    ...
    return ++count;
}, 0); //The variable is modified inside the function
//but its also used later to toggle its class

var twn_pointer_path = new TimelineMax({
    paused: true
});
createPointer(); //Adding it immediately triggers a bug for the transform
