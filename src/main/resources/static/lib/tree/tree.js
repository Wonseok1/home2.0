...
Tree.prototype.updateLiElementsOrgManage = function () {
  var _this6 = this;

  Object.values(this.willUpdateNodesById).forEach(function (node) {
    _this6.updateLiElementOrgManage(node);
  });
  this.willUpdateNodesById = {};
};
...
Tree.prototype.updateLiElementOrgManage = function (node) {
  ...
  if(node.status===0){
    ...
    if(document.getElementsByClassName("treejs-node__checked").length>0){
      let checked = document.getElementsByClassName("treejs-node__checked")
      let checkedList = []
      for(let i in checked){
        checkedList.push(checked[i].classList)
        checkedList[0].remove('treejs-node__halfchecked', 'treejs-node__checked');
      }
    }
